...
int main(int argc, char *argv[]) {
    ...
    string idmap_file_path = root_dir + "/sorted_sets/" + dataset_name + "_idmap.bin";
    string sortedsets_file_path = root_dir + "/sorted_sets/" + dataset_name + "_sortedsets.bin";
    ...
    if (dataset_name == "common_crawl") {
        ...
        getFiles(dataset_dir_path, cc_divided_filenames);
        for (auto const file_path : cc_divided_filenames) {
            loadTokensBasedOnIDX(file_path, docs);
            const string divided_dir_name = getDirectoryName(file_path);
            cout<< divided_dir_name<<endl;
            const string divided_dataset_name = dataset_name +"/"+ divided_dir_name;
            idmap_file_path = root_dir + "/sorted_sets/" + divided_dataset_name + "_idmap.bin";
            sortedsets_file_path = root_dir + "/sorted_sets/" + divided_dataset_name + "_sortedsets.bin";

#pragma omp parallel for
            for (int i = 0; i < docs.size(); ++i) {
                std::sort(docs[i].begin(), docs[i].end());
                docs[i].erase(std::unique(docs[i].begin(), docs[i].end()), docs[i].end());
                docs[i].shrink_to_fit();
            }
